...
<template>
  ...
  >
    <div class="flex items-center justify-between gap-3 mb-4">
      ...
      <span v-if="store.lastUpdated" class="text-sm text-text-muted">
    ...
    <div class="overflow-hidden rounded-lg border border-border">
      <table
        ...
        aria-label="Cryptocurrency prices in USD"
      >
        <thead class="bg-bg-input">
          <tr>
            <th
              class="text-left p-3 text-sm font-medium text-text-muted border-b border-border"
            >
              {{ t('prices.currency') }}
            </th>
            <th
    ...
    <p v-if="!store.lastUpdated" class="text-sm text-text-muted mt-3">
      {{ t('prices.waiting') }}
    </p>
  </div>
</template>
